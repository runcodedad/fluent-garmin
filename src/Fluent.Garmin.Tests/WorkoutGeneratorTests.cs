...
public class WorkoutGeneratorTests
{
    ...
    [Fact]
    public void GenerateWorkoutFileBytes_ShouldThrowExceptionForNullWorkout()
    {
        // Act & Assert
        Assert.Throws<ArgumentException>(() => 
            WorkoutGenerator.GenerateWorkoutFileBytes(null!));
    }

    [Fact]
    public void GenerateWorkoutFileBytes_ShouldThrowExceptionForEmptySteps()
    {
        // Arrange
        ...
        // Act & Assert
        Assert.Throws<ArgumentException>(() => 
            WorkoutGenerator.GenerateWorkoutFileBytes(workout));
    }

    [Fact]
    public void GenerateWorkoutFileBytes_ShouldReturnValidBytes()
    {
        // Arrange
        var workout = WorkoutGenerator.CreateIntervalWorkoutModel();

        // Act
        var bytes = WorkoutGenerator.GenerateWorkoutFileBytes(workout);

        // Assert
        Assert.NotNull(bytes);
        Assert.True(bytes.Length > 0);
        
        // Verify the bytes can be written to a file and are valid
        ...
        try
        {
            ...
            Assert.Equal(bytes.Length, new FileInfo(tempFile).Length);
        }
        finally
        {
    ...
    [Fact]
    public void GenerateWorkoutFileBytes_ShouldMatchFileOutput()
    {
        // Arrange
        var workout = WorkoutGenerator.CreateTempoRunModel();
        var tempFile = Path.GetTempFileName() + ".fit";
        ...
        try
        {
            // Act - Generate both bytes and file
            var bytes = WorkoutGenerator.GenerateWorkoutFileBytes(workout);
            WorkoutGenerator.GenerateWorkoutFile(workout, tempFile);
            var fileBytes = System.IO.File.ReadAllBytes(tempFile);

            // Assert - The bytes should be very close in length (within a few bytes due to timestamp differences)
            var lengthDifference = Math.Abs(fileBytes.Length - bytes.Length);
            Assert.True(lengthDifference <= 10, $"Length difference too large: {lengthDifference} bytes");
            Assert.True(bytes.Length > 100, "Generated bytes should be substantial");
        }
        finally
        {
            if (System.IO.File.Exists(tempFile))
